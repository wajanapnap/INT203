    ...
    <div class="md:hidden">
      <div class="w-full flex flex-col " v-for="(list,index) in lists " :key="index"> 
        <div class="border-2 rounded-md mb-2 mt-2 p-2" v-if="list.amount !==0"> 
          ...
          <div class="flex">Amount:<div :class="check(list.amount)" class="w-fit rounded-md ml-2">{{commas(list.amount)}}</div></div>
        </div>  
      </div>

      <div class="flex font-bold border-2 rounded-md p-2 pl-12">Net total:<div class="w-2/3 font-bold pl-12">{{commas(total)}}</div></div>
    </div>
      ...
      <table class="w-[45rem]">
        ...
        <tbody class="" v-for="(list,index) in lists" :key="index">
            <tr v-if="list.amount !==0">
              ...
              <td class="text-right py-1 px-2" ><div :class="check(list.amount)" class="w-fit ml-auto">{{commas(list.amount)}}</div></td>                 
            </tr>
        </tbody>
        <tr class="border-y-2 p-2">
            <td class="text-left font-bold px-2">Net total:</td>
            <td class="text-right font-bold px-2">{{commas(total)}}</td>
        </tr>
      </table>
    </div>
